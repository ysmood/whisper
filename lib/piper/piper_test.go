...
func TestEncodings(t *testing.T) {
	...
	enc, err := piper.Join(zstd, trans, piper.Join(aes, base64), trans).Encoder(buf)
	g.E(err)

	g.E(enc.Write([]byte("ok")))
	g.E(enc.Close())

	dec, err := piper.Join(zstd, piper.Join(aes, base64)).Decoder(buf)
	g.E(err)
